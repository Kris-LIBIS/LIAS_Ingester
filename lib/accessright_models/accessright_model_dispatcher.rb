...
class AccessrightModelDispatcher
  ...
  def set_model_map(ar_model_file)
    @ar_model_mapper = AccessrightModelMappper.new ar_model_file if ar_model_file
  end

  # !! overwrites custom data !!
  # Order of calling set_model_name and set_model_data is significant
  def set_model_name(ar_model_name)
    @ar_model = AccessrightModelFactory.instance.get_model ar_model_name if ar_model_name
  end
  ...
  def get_accessright_model(obj)
    ...
    if obj && @ar_model_mapper
      1.times do
        ...
        if mapped_model
          debug("Found accessright model '#{ar_model.name}' based on map file lookup.")
